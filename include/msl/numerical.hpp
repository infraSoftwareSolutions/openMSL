...
namespace msl
{
     ...
     */
    constexpr double ATE(double TrueValue, double ApproximateValue) noexcept { return abs(TrueValue - ApproximateValue); }
     ...
     */
    constexpr double AAE(double NewEstimate, double OldEstimate) noexcept { return abs(NewEstimate - OldEstimate); }
     ...
     * @param ApproximateValue The estimated or approximate value.
     * @return The relative true error as a double.
     */
    constexpr double RTE(double TrueValue, double ApproximateValue) noexcept { return abs(ATE(TrueValue, ApproximateValue) / TrueValue); }
     ...
     * @return The relative approximate error as a double.
     */
    constexpr double RAE(double NewEstimate, double OldEstimate) noexcept { return abs(AAE(NewEstimate, OldEstimate) / NewEstimate); }
     ...
     * @return The relative error threshold as a double.
     */
    constexpr double Es(int n) noexcept { return (0.5 * pow(10, 2 - n)); }

    /**
     * @brief Computes the square of a doubleing-point number.
     *
     * This function returns the result of raising the input value to the power of 2.
     *
     * @param x The doubleing-point number to be squared.
     * @return The square of the input value.
     *
     * @note This function is marked as constexpr and noexcept.
     */
    constexpr double square(double x) noexcept { return pow(x, 2); }
     ...
     * The roots are returned as a std::vector<double> containing two elements.
     * If the discriminant is negative, the results may be NaN due to sqrt of a negative number.
     *
     * @param a Coefficient of x^2.
     * @param b Coefficient of x.
     * @param c Constant term.
     * @return std::vector<double> A vector containing the two roots.
     */
    std::vector<double> QuadEquRoots(double a, double b, double c) noexcept
    {
     ...
     */
    constexpr double n(double a, double b, double E) noexcept { return (log(b - a) - log(E)) / log(2); }
     ...
     * @return double       The approximated root value, or NaN if the input is invalid or method fails.
     */
    double Bisection(std::function<double(double)> f, double Xl, double Xu, int iterations, double tolerance = 1e-5, bool ShowIter = false) noexcept
    {
        if (Xl > Xu)
        {
     ...
     * @param ShowIter     If true, prints each iteration step (default = false).
     * @return double       The estimated root, or the last Xi if convergence fails.
     */
    double SecantMethod(std::function<double(double)> f, double Xi, double Xi_1, int iterations, double tolerance = 1e-5, bool ShowIter = false) noexcept
    {
        double Xi_new;
        for (int i = 0; i < iterations; ++i)
        {
     ...
     * @return double    The approximated root value, or -1 if input is invalid.
     */
    double False_Position(std::function<double(double)> f, double Xl, double Xu, int iteration, bool ShowIter = false, double tolerance = 1e-5) noexcept
    {
        double Xr = Xl; // initial guess (just for setup)
        double Xr_old = Xr;
        double Fx_l = f(Xl), Fx_u = f(Xu), Fx_r;
        if (Xl > Xu)
        {
     ...
     * @return double     The approximated root.
     */
    double Newton_Raphson(std::function<double(double)> fx, std::function<double(double)> fd, double X_0, int iteration, bool ShowIter = false, double tolerance = 1e-5) noexcept
    {
        double Xi_old = X_0, Xi_new;
        for (int i = 0; i < iteration; i++)
        {
            double Fi_old = fx(Xi_old);
            double Fi_d = fd(Xi_old);
            if (Fi_d == 0)
            {
